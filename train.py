...
parser.add_argument('--checkpoint_path', type=str, default='/home/vandangorade/ISBI24/DKD/code/final_ckpts/lits/teacher')
parser.add_argument('--batch_size', type=int, default=8)
parser.add_argument('--mode', type=str, default='train')
parser.add_argument('--model', type=str, default='unet_r50')
parser.add_argument('--dataset', type=str, default='lits', choices=['kits', 'lits', 'heart'])
...
class SegPL(BasePLModel):
    ...
    def forward(self, x):
        e1,e2,e3,e4,e5, d5,d4,d3,d2,output = self.net(x)
        return output
...
def main():
    ...
    model = SegPL(args) #.load_from_checkpoint(checkpoint_path=os.path.join(args.checkpoint_path, 'checkpoint_lits_tumor_unet++_epoch=45.ckpt'))
    ...
    trainer = Trainer.from_argparse_args(args, 
                                        # resume_from_checkpoint=os.path.join(args.checkpoint_path, 'checkpoint_lits_tumor_unet++_epoch=45.ckpt'),
    ...
    # trainer = Trainer.from_argparse_args(args, resume_from_checkpoint=os.path.join(args.checkpoint_path, 'last.ckpt'), max_epochs=args.epochs, gpus=[4], callbacks=checkpoint_callback, logger=logger)
    trainer.fit(model)


def test():
    args = parser.parse_args()
    model = SegPL.load_from_checkpoint(checkpoint_path=os.path.join(args.checkpoint_path, 'checkpoint_lits_tumor_unet++_epoch=78.ckpt'))
    trainer = Trainer(gpus=[4, 5, 3],         
                    strategy='dp', 
                    precision=16)
    trainer.test(model)
